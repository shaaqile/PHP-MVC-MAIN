    ...
    <div class="col-lg-12">
      ...
      <ul class="list-group">
        <?php foreach ($data['all'] as $all) : ?>
          <li class="list-group-item">
            <?= $all['nama']; ?>
            <!-- <a href="<?= BASEURL; ?>/mahasiswa/hapus/<?= $all['id']; ?>" class="badge badge-danger float-right" onclick="return confirm('yakin?');">hapus</a>
            <a href="<?= BASEURL; ?>/mahasiswa/ubah/<?= $all['id']; ?>" class="badge badge-success float-right tampilModalUbah" data-toggle="modal" data-target="#formModal" data-id="<?= $mhs['id']; ?>">ubah</a>
            <a href="<?= BASEURL; ?>/mahasiswa/detail/<?= $all['id']; ?>" class="badge badge-primary float-right">detail</a> -->
            <?= $all['jurusan']; ?>
            <?= $all['namaMatkul']; ?>
            <?= $all['sks'] . 'sks'; ?>
